...
class ReviewFunnel(TenantBase):
    ...
    google_review_id: Mapped[str | None] = mapped_column(String(255), nullable=True, index=True)
    source: Mapped[str] = mapped_column(String(50), default="funnel")
    sentiment: Mapped[str | None] = mapped_column(String(20), nullable=True)
    assigned_to: Mapped[str | None] = mapped_column(String(36), nullable=True)
    assigned_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    reviewer_photo_url: Mapped[str | None] = mapped_column(String(512), nullable=True)
    google_reply: Mapped[str | None] = mapped_column(Text, nullable=True)
    google_reply_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    review_url: Mapped[str | None] = mapped_column(String(512), nullable=True)
